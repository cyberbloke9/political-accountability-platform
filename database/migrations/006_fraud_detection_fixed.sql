-- Migration: Fraud Detection System (FIXED)
-- Description: Track and flag suspicious activity, vote manipulation, and low-quality content
-- Date: 2025-11-23
-- FIX: Corrected column names (evidence_text, evidence_urls)
...
-- Function: Detect low-quality verifications (FIXED)
...
BEGIN
  ...
  FOR verification_record IN
    SELECT
      ...
      evidence_text,
      evidence_urls,
      LENGTH(evidence_text) as evidence_length
    FROM verifications
  ...
  LOOP
    ...
    -- Check if evidence URLs are missing
    IF verification_record.evidence_urls IS NULL OR array_length(verification_record.evidence_urls, 1) IS NULL THEN
      quality_score := quality_score + 0.15;
    END IF;

    -- Check for generic phrases indicating AI-generated content
    IF verification_record.evidence_text ~* '(as an ai|i cannot|i apologize|furthermore|moreover|in conclusion)' THEN
      quality_score := quality_score + 0.20;
    END IF;
    ...
    IF quality_score >= 0.70 THEN
      ...
      SELECT
        ...
        jsonb_build_object(
          ...
          'has_source', verification_record.evidence_urls IS NOT NULL AND array_length(verification_record.evidence_urls, 1) > 0,
...
-- Function: Detect duplicate evidence (plagiarism) (FIXED)
...
BEGIN
  ...
  LOOP
    ...
    FROM verifications
    WHERE evidence_text = verification_record.evidence_text
